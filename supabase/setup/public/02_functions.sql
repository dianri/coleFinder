-- ColeFinder – Funciones RPC (PROD / public)
-- Ejecutar después de public/01_schema.sql

-- =============================================================================
-- nearby_colegios — centros más cercanos (KNN sobre índice GIST)
-- =============================================================================
-- p_limit se conserva en la firma por compatibilidad con clientes antiguos,
-- pero se ignora: el límite real se lee de app_config.nearby_colegios_limit
-- (fallback 200, tope máximo 500).
...
COMMENT ON FUNCTION public.nearby_colegios(double precision, double precision, integer, text[], text[]) IS
  'RPC principal de la app: N centros más cercanos con filtros opcionales por titularidad y tipo.';

-- Permisos de ejecución para PostgREST (rol anon de la app)
GRANT EXECUTE ON FUNCTION public.nearby_colegios(
  double precision,
  double precision,
  integer,
  text[],
